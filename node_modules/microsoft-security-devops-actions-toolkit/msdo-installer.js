...
class MsdoInstaller {
    install(cliVersion) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!agentPackagesDirectory) {
                ...
                console.log(`agentPackagesDirectory = ${agentPackagesDirectory}`);
            ...
            console.log(`agentVersionsDirectory = ${agentVersionsDirectory}`);
            ...
            console.log(`msdoVersionsDirectory = ${msdoVersionsDirectory}`);
            ...
            do {
                ...
                console.log(`msdoToolkitDirectory = ${msdoToolkitDirectory}`);
                const msdoProjectFile = path.join(msdoToolkitDirectory, 'msdo-toolkit.proj');
                console.log(`msdoProjectFile = ${msdoProjectFile}`);
                let args = [
                    'restore',
                    msdoProjectFile,
    ...
    isInstalled(versionsDirectory, cliVersion) {
        ...
        if (cliVersion.includes("*")) {
            console.log(`MSDO CLI version contains a latest quantifier: ${cliVersion}. Continuing with install...`);
            return installed;
        }
    ...
    findLatestVersionDirectory(versionsDirectory, isPreRelease = false) {
        ...
        console.log(`Searching for all version folders in: ${versionsDirectory}`);
        let dirs = this.getDirectories(versionsDirectory);
        for (let dirIndex = 0; dirIndex < dirs.length; dirIndex++) {
            let dir = dirs[dirIndex];
            if (dir == null || dir == "") {
                console.log(`Skipping null or empty directory: ${dir}`);
                continue;
            }
            console.log(`Evaluating MSDO directory: ${dir}`);
            const dirRegex = new RegExp(/^(\d+\.?){1,6}(\-\w+)?$/g);
            if (dirRegex.exec(dir) == null) {
                console.log(`Skipping invalid version directory: ${dir}`);
                continue;
            }
            ...
            if (!isPreRelease && dirIsPreRelease) {
                console.log(`Skipping pre-release version directory: ${dir}`);
                continue;
            }
            ...
            if (isLatest) {
                console.log(`Setting latest version directory: ${dir}`);
        ...
        console.log(`latestDirectory = ${latestDirectory}`);
    ...
    setVariables(packageDirectory) {
        ...
        console.log(`msdoDirectory = ${msdoDirectory}`);
